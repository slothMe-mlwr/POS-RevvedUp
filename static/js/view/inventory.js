 ...
 // Tabs
    ...
    function applyFilters() {
      ...
      $('#productTableBody tr').each(function () {
        ...
        // Adjust index offsets based on role
        const offset = userPosition === "employee" ? -1 : 0;
    
        const rowSpeed = row.find(`td:eq(${5 + offset})`).text().trim().split(' ')[0] + ' ' + row.find(`td:eq(${5 + offset})`).text().trim().split(' ')[1];
        const rowCategory = row.find(`td:eq(${6 + offset})`).text().trim();
        const rowStatusColor = row.find(`td:eq(${7 + offset}) span`).attr('class') || "";
                        ...
                        res.data.forEach(data => {
                            ...
                            $('#productTableBody').append(`
                                <tr>
                                    ...
                                    ${isAdmin ? `<td class="px-4 py-2 align-middle whitespace-nowrap">₱ ${data.prod_capital}</td>` : ''}
                                    <td class="px-4 py-2 align-middle whitespace-nowrap">₱ ${data.prod_price}</td>
