...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['requestType'])) {
         if ($_POST['requestType'] == 'Login_user') {
        ...
        } else if ($_POST['requestType'] == 'mark_as_seen') {
              if (isset($_POST['notifID'])) {
                    $ids = $_POST['notifID'];
                    $result = $db->mark_as_seen($ids); // call class method
                    ...
                    echo json_encode(['status' => 400, 'message' => 'No notification IDs provided']);
                }
                exit;
                ...
                $result = $db->add_user($nickname, $username, $email, $password);
                ...
                $result = $db->update_user($userId, $nickname, $username, $email);
            ...
            if ($result['success']) {
                ...
                $errorMsg = null;
                $result = $db->CheckOutOrder($services, $items, $discount, $vat, $grandTotal, $payment, $change, $user_id, $errorMsg);
        ...
        }else if($_POST['requestType'] === 'complete_exchange'){
            $transactionId = $_POST['transactionId'];
            $exchange = json_decode($_POST['exchange'], true);
            ...
            // Pass data to database function
            $result = $db->complete_exchange($transactionId, $exchange);
        ...
        } else if ($_POST['requestType'] == 'mark_seen_notifications') {

            $ids = $_POST['notifIds'] ?? [];
            $status = $db->mark_notifications_seen($ids);
                echo json_encode(['status' => $status ? 200 : 500]);
                //--------------------------------------------------------------------------------------------------------------
   ...
   if (isset($_GET['requestType']))
    {
        if ($_GET['requestType'] == 'fetch_all_product') {
            ...
            } else if ($_GET['requestType'] == 'fetch_available_products') {
                $result = $db->fetch_available_products();
                echo json_encode([
                    'status' => 200,
                    'data' => $result
                ]);
            ...
            }else if ($_GET['requestType'] === 'fetch_notifications') {

    $result = $db->fetch_notifications();
    echo json_encode([
        'status' => 200,
        'data' => $result
    ]);
...
// -----------------------------
} else if ($_GET['requestType'] === 'fetch_restock_alerts') {
    ...
    $result = $db->fetch_restock_alerts();
    echo json_encode([
        'status' => 200,
        'data' => $result,
        'position' => $position
...
// -----------------------------
// 🔢 FETCH UNREAD RESTOCK ALERT COUNT
// -----------------------------
        } else if ($_GET['requestType'] == 'getRestockAlertCount') {

            $result = $db->getRestockAlertCount();
            echo json_encode([
                'status' => 200,
                'data' => $result
            ]);
        //--------------------------------------------------------------------------------------------------------------
        } else if ($_GET['requestType'] === 'checkFastMovingAlerts') {
            $result = $db->fetch_restock_alerts();
            echo json_encode([
                'status' => 200,
                'data' => $result
        ...
        } else if ($_GET['requestType'] === 'markNotificationsSeen') {

            // Expecting an array of notif IDs
            $ids = isset($_POST['ids']) ? $_POST['ids'] : [];

            if ($db->mark_notifications_seen($ids)) {
                echo json_encode([
                    'status' => 200,
        ...
        // FETCH ALL SERVICES (for search)
        }   else if ($_GET['requestType'] == 'fetch_all_service') {
            $result = $db->fetch_all_service();
            echo json_encode([
                'status' => 200,
                'data' => $result
            ]);
        ...
        } else if ($_GET['requestType'] == 'fetch_available_employees') {
            $date = $_GET['appointmentDate'] ?? null;
            $time = $_GET['appointmentTime'] ?? null;
            ...
            $result = $db->fetch_available_employees($date, $time);

            if ($result['fully_booked'] === true) {
                echo json_encode([
                    'status' => 403,
                    'message' => 'This timeslot is fully booked. Please choose another time.',
